...
public:
    ...
    {
        // maiacoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // maiacoin: get transaction coin age
...
public:
    ...
    // maiacoin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // maiacoin: two types of block: proof-of-work or proof-of-stake
    ...
    // maiacoin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // maiacoin: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // maiacoin: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // maiacoin: block index flags
    enum  
    {
